...
**No genero una captura real en esta fase.** Implemento el componente `ProductScreenshot.tsx` con un placeholder claramente identificado (marco con etiqueta "Real product view — screenshot pending", proporciones correctas para desktop/móvil, sin inventar contenido de dashboard dentro del placeholder). Documento en el propio componente y en el informe final exactamente qué captura hace falta, de qué pantalla, con qué usuario (uno con datos reales y sin información de facturación visible), y dónde colocar el archivo (`frontend/public/pricing/dashboard-preview.png`, referenciado desde `ProductScreenshot.tsx`).
